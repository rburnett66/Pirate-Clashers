...
export async function playSpecialScene({field,move,pending,getBattle,getCamera,reduced,onImpact,onAfterImpact,onCelebrate,sound,rigImage}){
 ...
 const victimImage=victim?await load('/Pirate%20Art/pirate_segments/zombie-pirate/zombie-pirate-'+victim.id+'.png'):null;
 const reacting= [5,6].includes(move.id)?await Promise.all(pending.before.crew.filter(g=>g.hp>0&&g.slot.startsWith('d')).map(async g=>({g,img:await load('/Pirate%20Art/pirate_segments/zombie-pirate/zombie-pirate-'+g.id+'.png')}))):[];
