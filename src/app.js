...
function title(k,h,p=''){const banner={battle:'battle',crew:'crew',booty:'booty',store:'store'}[page];return '<div class="page-title"><div class="page-heading">'+(banner?artImage(banner,'title-art',''):'')+'<div><div class="eyebrow">'+k+'</div><h2>'+h+'</h2></div></div><p>'+p+'</p></div>';}
...
function shell(){
 document.body.classList.toggle('reduce-motion',!state.settings.motion);document.body.classList.toggle('battle-mode',page==='arena');document.body.dataset.page=page;
 ...
 el('rail').innerHTML='<div class="brand"><span>☠</span><small>PIRATE<br>CLASHERS</small></div>'+nav.map(([k,i,n])=>'<button class="nav-btn '+(page===k?'active':'')+'" data-action="nav" data-id="'+k+'" aria-label="'+n+'" '+(page===k?'aria-current="page"':'')+'>'+artImage(i,'nav-art')+'<span>'+n+'</span></button>').join('')+'<div class="version">LOCAL VOYAGE</div>';
 el('topbar').innerHTML='<div class="captain-mark"><span class="eyebrow">PIRATE CLASHERS</span><strong>'+esc(state.name)+'</strong><small>'+M.PORTS[state.port]+' · Local voyage</small></div><div class="wallet-plaque">'+artImage('header','header-art')+'<div class="wallet"><span class="wallet-label">YOUR TREASURE</span><span class="currency" aria-label="'+fmt(state.gold)+' gold"><small>GOLD</small><strong>'+fmt(state.gold)+'</strong></span><button class="wallet-add" data-action="nav" data-id="store" aria-label="Open gold and gem store">+</button><span class="gem" aria-label="'+fmt(state.gems)+' gems"><small>GEMS</small><strong>'+fmt(state.gems)+'</strong></span></div></div>'+button('How to play','help','','header-help ghost small');
}
...
function home(){const roster=Object.values(state.slots).map(pirate),view=harbor(state.settings.harbor);return title('READY THE CREW','Your next adventure awaits.','Season of the Drowned Crown')+
 '<div class="harbor-layout"><section class="harbor-main"><div class="harbor-scene brass-panel">'+artImage(view.id,'harbor-image',view.name+' harbor artwork')+'<div class="harbor-caption"><span class="eyebrow">'+esc(view.mood)+'</span><h1>'+view.name+'</h1><span class="pill">⚓ Your port: '+M.PORTS[state.port]+'</span></div><div class="sail-action">'+button(state.battle&&state.battle.phase!=='result'?'Resume battle →':artImage('find','','Find match'),'start','','art-button')+'<small>Local battles · seeded rivals</small></div></div><div class="harbor-picker" aria-label="Harbor scenery">'+HARBORS.map(h=>'<button data-action="harbor" data-id="'+h.id+'" class="harbor-choice '+(view.id===h.id?'active':'')+'" aria-pressed="'+(view.id===h.id)+'">'+artImage(h.id,'',h.name)+'<span>'+h.name+'</span></button>').join('')+'</div><p class="scenery-note">Choose your harbor scenery</p></section>'+
 '<aside class="harbor-aside"><section class="card"><div class="row between"><h3>Your fighting crew</h3>'+button('Manage →','nav','crew','ghost small')+'</div><div class="crew-dots">'+roster.map(p=>'<button class="crew-preview" data-action="detail" data-id="'+p.id+'" aria-label="View '+esc(p.name)+'">'+portrait(p)+'<span>'+esc(p.name)+'</span></button>').join('')+'</div><p class="footer-note">'+roster.length+' gunners aboard · Ship level '+state.shipLevel+'</p>'+button('Prepare your ship','nav','crew','small full-width')+'</section><section class="card"><div class="row between"><h3>Captain’s orders</h3><span class="pill">DAILY</span></div><div class="quest"><div class="row between"><span>Finish 3 battles</span><span>'+Math.min(3,state.quests.matches)+'/3</span></div><progress max="3" value="'+state.quests.matches+'"></progress>'+button(state.quests.dailyClaim?'Collected ✓':'Claim 100 season XP','quest','daily','small',state.quests.dailyClaim||state.quests.matches<3)+'</div><div class="rule"></div><div class="row between"><div><span class="eyebrow">PIRATE’S BOOTY</span><h3>Level '+M.seasonLevel(state)+'</h3></div>'+button('Rewards →','nav','booty','ghost small')+'</div><progress class="spaced" max="100" value="'+state.xp%100+'"></progress><small>'+(state.xp%100)+' / 100 XP to next level</small></section></aside></div>'+
 (state.lastResult&&(!state.battle||state.battle.phase==='result')?'<div class="last-battle row between"><span>Last battle · '+(state.lastResult.won?'Victory':'Defeat')+' against '+esc(state.lastResult.enemyName)+'</span>'+button('View battle results','last-result','','small')+'</div>':'')+'<section class="card hold-panel spaced"><div class="row between"><div class="row">'+artImage('open','hold-title','Open booty chest')+'<div><span class="eyebrow">THE HOLD</span><h3>Your hard-earned haul</h3></div></div><small>'+state.dailyChests+' / 5 chest drops today</small></div><div class="chests spaced">'+Array.from({length:5},(_,i)=>{const c=state.chests[i];return c?button(artImage('chest','chest-art')+'<strong>'+c.kind+'</strong><small>Open chest</small>','chest',c.id,'chest'):'<div class="chest empty-chest">'+artImage('chest','chest-art')+'<small>Win a battle</small></div>';}).join('')+'</div></section>';}
...
function diagram(){const valid=M.positions(state);return '<div class="ship-diagram"><div class="mast"></div><div class="sail-drawing">☠</div><div class="ship-outline"></div>'+['d0','d1','d2','d3','h0','h1','h2','h3'].map((key,i)=>{const p=state.slots[key]&&pirate(state.slots[key]),open=valid.includes(key);return '<button class="slot '+(i>3?'port ':'')+(!open?'locked':'')+'" style="left:'+(24+i%4*17)+'%;top:'+(i<4?43:66)+'%" data-action="slot" data-id="'+key+'" aria-label="'+(open?(p?p.name:'Empty')+' '+(i<4?'deck':'gun port')+' station '+(i%4+1):'Locked station')+'" '+(!open?'disabled':'')+'>'+(open?p?portrait(p):'+':'⌑')+'</button>';}).join('')+'</div>';}
...
case 'harbor':if(HARBORS.some(h=>h.id===id)){state.settings.harbor=id;save();render();}break;
