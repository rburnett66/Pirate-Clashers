...
function shell(){
 ...
 el('rail').innerHTML='<div class="brand"><span>☠</span><small>PIRATE<br>CLASHERS</small></div>'+nav.map(([k,i,n])=>'<button class="nav-btn '+(page===k?'active':'')+'" data-action="nav" data-id="'+k+'" aria-label="'+n+'" '+(page===k?'aria-current="page"':'')+'><span class="nav-art-slot">'+artImage(i,'nav-art')+'</span><span>'+n+'</span></button>').join('')+'<div class="version">LOCAL VOYAGE</div>';
...
function home(){
 ...
 const choices=HARBORS.map(h=>`<button data-action="harbor" data-id="${h.id}" class="harbor-choice ${view.id===h.id?'active':''}" aria-pressed="${view.id===h.id}">${artImage(h.id,'',h.name)}<span>${h.name}</span></button>`).join('');
 ...
 return title('READY THE CREW','Your next adventure awaits.','Season of the Drowned Crown')+`
 <div class="harbor-layout">
  <section class="harbor-main"><div class="harbor-scene brass-panel">
   ${artImage(view.id,'harbor-image',view.name+' harbor artwork')}
   <div class="harbor-caption"><span class="eyebrow">${esc(view.mood)}</span><h1>${view.name}</h1><span class="pill">⚓ Your port: ${M.PORTS[state.port]}</span></div>
  ...
  <aside class="harbor-aside">
   <section class="card crew-overview"><div class="row between"><h3>Your fighting crew</h3>${button('Manage →','nav','crew','ghost small')}</div>
    ...
    <p class="footer-note">${roster.length} gunners aboard · Ship level ${state.shipLevel}</p>${button('Prepare your ship','nav','crew','small full-width')}
    <div class="scenery-selector"><p class="scenery-note">Choose your harbor scenery</p><div class="harbor-picker" aria-label="Harbor scenery">${choices}</div></div>${recent}
   </section>
