...
(async()=>{
 ...
 try{
  ...
  await page.evaluate(async()=>{const M=await import('/src/model.js');window.fixture=M.startBattle(M.fresh()).player;});
  const configure=async(cosmetic,level=3)=>{
   ...
   await page.waitForFunction(()=>document.body.dataset.shipArt==='ready');await page.waitForTimeout(150);
  };
  ...
  await configure(3,8);assert.equal(await page.evaluate(()=>document.body.dataset.hullArt),'IMG_7300');await page.screenshot({path:'test-results/ship-art-white-sails.png'});
  ...
  await page.evaluate(()=>window.postMessage({type:'pirate-render',action:'view',cutaway:true,hideRig:true},location.origin));await page.waitForTimeout(200);assert.ok((await pixels('#gameHullArt')).alpha<closed.alpha);
  ...
  assert.deepEqual(errors,[]);console.log('Ship art checks passed: six distinct sail sets, level-eight hull, visible sail damage, all mast/sail/flag groups removed together, and reversible interior reveal.');
