...
await page.evaluate(async()=>{const M=await import('/src/model.js'),s=JSON.parse(localStorage.getItem('pirate-clashers-v1'));const b=s.battle;let angle=5,hit;for(;angle<=75;angle++){hit=M.previewShot(s,{gunner:1,angle}).shots[0].impact;if(hit?.kind==='hull')break;}if(!hit||angle>75)throw Error('No fixture hull trajectory');b.aimAngle=angle;const H=await import('/src/hull-mask.js'),pixels=H.maskPixels(b.enemy).slice();for(let i=0;i<pixels.length;i++){const p=H.pixelPoint(i%H.HULL_MASK.width,Math.floor(i/H.HULL_MASK.width));if(Math.hypot(p.x-hit.x,p.y-hit.y)>.025)pixels[i]=0;}H.replaceHullPixels(b.enemy,pixels);b.rng=1;localStorage.setItem('pirate-clashers-v1',JSON.stringify(s))});
