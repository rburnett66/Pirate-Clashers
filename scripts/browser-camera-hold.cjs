...
(async()=>{
 ...
 try{
  ...
  await page.waitForFunction(()=>{const b=JSON.parse(localStorage.getItem('pirate-clashers-v1')).battle;return b.pending?.side==='enemy';},null,{timeout:15000});await page.waitForTimeout(100);assert.equal(await frame.evaluate(()=>window.testCutaway),false);
  ...
  await page.screenshot({path:'test-results/camera-hold-after-enemy.png'});assert.deepEqual(errors,[]);console.log('Camera tests passed: immediate selection zoom-out, 3-second hit hold, paused turn clock, retained wide view, and closed player hull during enemy shots.');
