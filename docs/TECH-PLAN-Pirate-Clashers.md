...
Core GDD and deconstruct are missing: document local assumptions rather than invent their text. Baseline two player shots per turn, local game state owns the hull material mask shared by rendering and collision; decorative GPU effects remain cosmetic, hull/crew victory and 24-turn cap. Retain proposals as configurable defaults: 50 season levels, one finisher after four consecutive dual-hit turns, simulated rematch 65%/-20%, one per loss. Season duration/price, queue/duplicate policy, durable equipment tuning and season rewards use explicit local defaults in docs/LOCAL-BUILD.md. Ship upgrades are materials only.
